...
{
  "id": "sec-app-functions",
  "level": "1",
  "url": "sec-app-functions.html",
  "type": "Section",
  "number": "B.1",
  "title": "Functions",
  "body": " Functions    A function  is an assignment between sets and satisfying the condition that for all , if in , then in i.e., is well-defined. The set is called the domain of and the set is called the codomain of . The subset of the codomain that can be obtained as for some in the domain is called the range of , denoted   Given a function , we say that is injective (equivalently, one-to-one ) provided for all , if , then .  Given a function , we say that is surjective (equivalently, onto ) provided for every , there exists satisfying .  A function that is both injective and surjective is called bijective (equivalently, a bijection ).      Recall that   The assignment given by satisfying is not a function. Indeed, is undefined for ; in this sense, the domain is too big. Even restricting the domain to nonnegative inputs, we find that is ambiguous, because both and satisfy .  The assignment given by is a function. Restricting the domain to ensures each input has at least one output, and restricting the codomain to ensures that output is unique.     Complex Conjugation   The complex numbers are defined to be the set with addition defined by collecting like terms and multiplication defined using the distributive property and the rule : and Two complex numbers and are equal provided and . When we write , we understand it as a shorthand for .  We will verify that the assignment given by is a bijective function.  (Function.) Recall, our goal is to show that the same input to yields the same output, even if we can write the input in cosmetically different ways. So, we'll start with an input element written in two ways: suppose in the domain of . This equality implies that and . This means , too, which ensures that in the codomain of , as desired..  (Injective.) In this case, our goal is to show that if input elements yield the same outputs, the inputs were the same to begin with. So, we'll start with two elements that give the same output: suppose in the codomain. This implies that , which means , , and hence . Thus in the domain, as desired.  (Surjective.) Now our goal is to show that given an arbitrary element of the codomain, we can find an element of the domain that maps to it. So, we'll start with a complex number in the codomain: suppose . It follows that is an element of the domain for which as desired.      Given functions and , their composition is defined to be the assignment given by the rule .      The composition of functions with compatible domains and codomains is a function. That is, if and are functions, then is a function as well.  Furthermore, if both and are injective, then is injective.  Similarly, if both and are surjective, then is surjective.     Suppose and are functions. Let , .  If , then because is a function. Similarly, implies because is a function. It follows that implies and therefore is a function.  Suppose now that is injective. This means that if , we have that . Since is injective, , and since is injective, . Now we have that implies , so is injective.  The rest of the proof is left as an exercise to the reader.     Logical Relationships   Consider the statements, Statement A: if in , then in and Statement B: if in , then in .      What is the logical relationship between Statements A and B?      What is the contrapositive of Statement B?      Rewrite the definition injective using the contrapositive of Statement B.      Write the negation of the definition of injective .      Write the negation of the definition of surjective .      A Little Trigonometry Review   Consider the function given by . Recall that refers to the open interval of real numbers with values between and . That is,      Identify the domain and codomain of .      Is injective? Justify your answer.      Is injective? Justify your answer.      Determine a domain and codomain for which the function defined by is a bijection.       Complete the unfinished proof of the theorem.     "
},
{
  "id": "def-function",
  "level": "2",
  "url": "sec-app-functions.html#def-function",
  "type": "Definition",
  "number": "B.1.1",
  "title": "",
  "body": "  A function  is an assignment between sets and satisfying the condition that for all , if in , then in i.e., is well-defined. The set is called the domain of and the set is called the codomain of . The subset of the codomain that can be obtained as for some in the domain is called the range of , denoted   Given a function , we say that is injective (equivalently, one-to-one ) provided for all , if , then .  Given a function , we say that is surjective (equivalently, onto ) provided for every , there exists satisfying .  A function that is both injective and surjective is called bijective (equivalently, a bijection ).   "
},
{
  "id": "sec-app-functions-3",
  "level": "2",
  "url": "sec-app-functions.html#sec-app-functions-3",
  "type": "Example",
  "number": "B.1.2",
  "title": "",
  "body": "  Recall that   The assignment given by satisfying is not a function. Indeed, is undefined for ; in this sense, the domain is too big. Even restricting the domain to nonnegative inputs, we find that is ambiguous, because both and satisfy .  The assignment given by is a function. Restricting the domain to ensures each input has at least one output, and restricting the codomain to ensures that output is unique.   "
},
{
  "id": "sec-app-functions-4",
  "level": "2",
  "url": "sec-app-functions.html#sec-app-functions-4",
  "type": "Example",
  "number": "B.1.3",
  "title": "Complex Conjugation.",
  "body": " Complex Conjugation   The complex numbers are defined to be the set with addition defined by collecting like terms and multiplication defined using the distributive property and the rule : and Two complex numbers and are equal provided and . When we write , we understand it as a shorthand for .  We will verify that the assignment given by is a bijective function.  (Function.) Recall, our goal is to show that the same input to yields the same output, even if we can write the input in cosmetically different ways. So, we'll start with an input element written in two ways: suppose in the domain of . This equality implies that and . This means , too, which ensures that in the codomain of , as desired..  (Injective.) In this case, our goal is to show that if input elements yield the same outputs, the inputs were the same to begin with. So, we'll start with two elements that give the same output: suppose in the codomain. This implies that , which means , , and hence . Thus in the domain, as desired.  (Surjective.) Now our goal is to show that given an arbitrary element of the codomain, we can find an element of the domain that maps to it. So, we'll start with a complex number in the codomain: suppose . It follows that is an element of the domain for which as desired.   "
},
{
  "id": "def-composition",
  "level": "2",
  "url": "sec-app-functions.html#def-composition",
  "type": "Definition",
  "number": "B.1.4",
  "title": "",
  "body": "  Given functions and , their composition is defined to be the assignment given by the rule .   "
},
{
  "id": "thm-composition",
  "level": "2",
  "url": "sec-app-functions.html#thm-composition",
  "type": "Theorem",
  "number": "B.1.5",
  "title": "",
  "body": "  The composition of functions with compatible domains and codomains is a function. That is, if and are functions, then is a function as well.  Furthermore, if both and are injective, then is injective.  Similarly, if both and are surjective, then is surjective.   "
},
{
  "id": "sec-app-functions-7",
  "level": "2",
  "url": "sec-app-functions.html#sec-app-functions-7",
  "type": "Proof",
  "number": "B.1.1",
  "title": "",
  "body": " Suppose and are functions. Let , .  If , then because is a function. Similarly, implies because is a function. It follows that implies and therefore is a function.  Suppose now that is injective. This means that if , we have that . Since is injective, , and since is injective, . Now we have that implies , so is injective.  The rest of the proof is left as an exercise to the reader.  "
},
{
  "id": "exercises-functions-1-3",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-1-3",
  "type": "Exercise",
  "number": "B.1.1",
  "title": "",
  "body": "  What is the logical relationship between Statements A and B?   "
},
{
  "id": "exercises-functions-1-4",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-1-4",
  "type": "Exercise",
  "number": "B.1.2",
  "title": "",
  "body": "  What is the contrapositive of Statement B?   "
},
{
  "id": "exercises-functions-1-5",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-1-5",
  "type": "Exercise",
  "number": "B.1.3",
  "title": "",
  "body": "  Rewrite the definition injective using the contrapositive of Statement B.   "
},
{
  "id": "exercises-functions-1-6",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-1-6",
  "type": "Exercise",
  "number": "B.1.4",
  "title": "",
  "body": "  Write the negation of the definition of injective .   "
},
{
  "id": "exercises-functions-1-7",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-1-7",
  "type": "Exercise",
  "number": "B.1.5",
  "title": "",
  "body": "  Write the negation of the definition of surjective .   "
},
{
  "id": "exercises-functions-2-3",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-2-3",
  "type": "Exercise",
  "number": "B.1.6",
  "title": "",
  "body": "  Identify the domain and codomain of .   "
},
{
  "id": "exercises-functions-2-4",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-2-4",
  "type": "Exercise",
  "number": "B.1.7",
  "title": "",
  "body": "  Is injective? Justify your answer.   "
},
{
  "id": "exercises-functions-2-5",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-2-5",
  "type": "Exercise",
  "number": "B.1.8",
  "title": "",
  "body": "  Is injective? Justify your answer.   "
...
{
  "id": "exercises-functions-2-6",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-2-6",
  "type": "Exercise",
  "number": "B.1.9",
  "title": "",
  "body": "  Determine a domain and codomain for which the function defined by is a bijection.   "
},
{
  "id": "exercises-functions-3",
  "level": "2",
  "url": "sec-app-functions.html#exercises-functions-3",
  "type": "Exercise",
  "number": "B.1.10",
  "title": "",
  "body": "  Complete the unfinished proof of the theorem.   "
},
{
  "id": "sec-app-sets-equivalence-relations",
  "level": "1",
  "url": "sec-app-sets-equivalence-relations.html",
  "type": "Section",
  "number": "B.2",
  "title": "Equivalence relations on sets",
  "body": " Equivalence relations on sets  Equivalence relations are meant to answer the question, When are two elements of this set the same relative to some special property? The most common equivalence relation is numerical equality: two numbers are equal if they have the same value.    Let be a set. An equivalence relation on is a relation on ordered pairs satisfying the following properties:   Reflexivity: for all , ;    Symmetry: for all , implies ; and    Trransitivity: for all , and implies .    We define the equivalence class of to be , and we define the set of equivalence classes modulo to be .     Given integers where are nonzero, we say provided .  To verify that is an equivalence relation, we'll check the three properties. To start, suppose are integers where are nonzero. For reflexivity, observe because .  For symmetry, observe that implies because by the commutativity of integer multiplication our starting assumption.  For transitivity, observe that and implies because and imply , which implies (recall, by assumption).  This equivalence relation is important! When we talk about the rational numbers, we are really talking about equivalence classes of ordered pairs of integers modulo , where the first element of the ordered pair is the what we usually call the numerator of the rational number, and the second (nonzero) element of the ordered pair is what we usually call the denominator.    "
},
{
  "id": "sec-app-sets-equivalence-relations-3",
  "level": "2",
  "url": "sec-app-sets-equivalence-relations.html#sec-app-sets-equivalence-relations-3",
  "type": "Definition",
  "number": "B.2.1",
  "title": "",
  "body": "  Let be a set. An equivalence relation on is a relation on ordered pairs satisfying the following properties:   Reflexivity: for all , ;    Symmetry: for all , implies ; and    Trransitivity: for all , and implies .    We define the equivalence class of to be , and we define the set of equivalence classes modulo to be .  "
},
{
  "id": "sec-app-sets-equivalence-relations-4",
  "level": "2",
  "url": "sec-app-sets-equivalence-relations.html#sec-app-sets-equivalence-relations-4",
  "type": "Example",
  "number": "B.2.2",
  "title": "",
  "body": "  Given integers where are nonzero, we say provided .  To verify that is an equivalence relation, we'll check the three properties. To start, suppose are integers where are nonzero. For reflexivity, observe because .  For symmetry, observe that implies because by the commutativity of integer multiplication our starting assumption.  For transitivity, observe that and implies because and imply , which implies (recall, by assumption).  This equivalence relation is important! When we talk about the rational numbers, we are really talking about equivalence classes of ordered pairs of integers modulo , where the first element of the ordered pair is the what we usually call the numerator of the rational number, and the second (nonzero) element of the ordered pair is what we usually call the denominator.   "
},
{
  "id": "backmatter-4",
  "level": "1",
